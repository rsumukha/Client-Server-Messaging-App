...
def checkauth():
    while True:
        ...
        for client in arr:
            endtime = time.time()
            with auth_lock:
                if endtime-authdict[client] > 600:
    ...
    while True:
        ...
        if data[2] == util.OPCODE_LOGIN:
            ...
            if data[2] == util.OPCODE_LOGOUT_ACK:
                ...
                print("Client has posted a picture.")
                tweet = data[8: length].decode('utf-8')
                incomingtcpbufferlength = int(tweet)
                subscribers = subdict[currentclient]

                create_destroy_thread(handle_pic_post, incomingtcpbufferlength,currentclient)
                for sub in subscribers:
                    sendack(util.OPCODE_POSTPICMSG, str(incomingtcpbufferlength),getClient(sub))
                create_destroy_thread(send_all_followers, incomingtcpbufferlength,currentclient)
...
def create_destroy_thread(f, arg1, arg2):
    tcp = threading.Thread(target=f, args=(arg1,arg2,))
    tcp.start()
    tcp.join()

def send_all_followers(bufferlen, curclient):
    time.sleep(1)
    with open("/home/sumukha/CSE434/assignment4/server/"+curclient+".jpg", 'rb') as a:
        data = a.read()
    for sub in subdict[curclient]:
        print("\nForwarding the picture to "+sub)
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as skt:
            skt.connect(getClient(sub))
            skt.sendall(data)

def handle_pic_post(bufferlen, curclient):
    print("Saving the Picture..")
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as skt:
        skt.bind(util.SERVER_ADDR)
        skt.listen()
    ...
    with open("/home/sumukha/CSE434/assignment4/server/"+curclient+".jpg", 'wb+') as a:
        a.write(data)
